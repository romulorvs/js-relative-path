...
const changeActiveItem_1 = require("./utils/changeActiveItem");
...
var getDirectories = function (callback) {
    return __awaiter(this, void 0, void 0, function* () {
        if (projectRoot) {
            ...
            for (let index = 0; index < deepness; index++) {
                ignoreLevels += '/*';
            }
            ignoreLevels += '*';
            try {
...
var storedItems = [];
var storedItemsFilter = [];
var storedFiles = [];
var storedDirs = {};
var selectionFilesIndex = {};
var currentFileIndex = -1;
var isFiltering = null; // null means the first filtering
quickPick.busy = true;
    ...
    if (projectRoot && openedFilename) {
        ...
        if (typeof selectionFilesIndex[openedFileId] !== 'undefined') {
            ...
            if (currentFileIndex >= 0) {
                changeActiveItem_1.default(quickPick, currentFileIndex);
            }
        }
    }
}
...
const getImportTypeLabel = () => (importType === 'require'
...
function setItemsFromFiles(files = storedFiles) {
    ...
    files.forEach(function (file) {
        ...
        var extLastIndex = filename.lastIndexOf('.');
    ...
    fileData.forEach(function ({ label, dir, file, isDir, ext }, index) {
        ...
        dirNoSlasheAtEdge = (dirNoSlasheAtEdge[dirNoSlasheAtEdge.length - 1] === '/'
            ? dirNoSlasheAtEdge.slice(0, -1)
            : dirNoSlasheAtEdge);
        dirNoSlasheAtEdge = (dirNoSlasheAtEdge[0] === '/'
...
function normalizeFilename(fileWithoutExt, fileExtension) {
...
function importFile(file, isDir = false) {
    if (isDir) {
        file = `${file}index.js`;
    }
    if (!openedFilename) {
    ...
    quickPick.hide();
    activeTextEditor === null || activeTextEditor === void 0 ? void 0 : activeTextEditor.edit(function (editor) {
        if (!activeTextEditor) {
            return;
        }
        ...
        if ((fileType === 'javascript' && ['js'].includes(fileExtension)) ||
        ...
        if (lineText.trim()) { // if current line is not empty
            ...
            activeTextEditor.selection = new vscode_1.Selection(lineNumber, firstCharPos, lineNumber, lastCharPos);
    ...
    if (storedItems && storedItems.length > 0) {
        ...
        if (item.filename === '%%import_change%%') {
            if (importType === 'require') {
                ...
                vscode_1.window.showInformationMessage('using "Import...From"');
            }
            else {
                setImportType('require');
                vscode_1.window.showInformationMessage('using "Require"');
            }
            const firstStoredItem = Object.assign(Object.assign({}, item), { label: getImportTypeLabel() });
...
function runImportSelector() {
    pickImportType.items = [
        { label: 'import...from', type: 'import' },
        { label: 'require', type: 'require' },
    ];
    pickImportType.show();
}
const runExtension = () => {
    if (!projectRoot) {
        var [newProjectRoot, newOpenedFilename] = getPaths_1.default();
        projectRoot = newProjectRoot;
        openedFilename = newOpenedFilename;
    }
    else {
        var newOpenedFilename = getPaths_1.default()[1];
        openedFilename = newOpenedFilename;
    }
    quickPick.show();
    loadItems();
...
function activate(extContext) {
    ...
    context.subscriptions.push(vscode_1.commands.registerCommand('js-relative-import.showinput', () => {
        if (!importType) {
            runImportSelector();
        }
        else {
